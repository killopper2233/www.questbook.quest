...
class Webnus_Element_Widgets_Food_Menu extends \Elementor\Widget_Base {
	...
	public function get_title() {
		...
		return esc_html__( 'Food Menu', 'deep' );
	...
	public function get_icon() {
		...
		return 'deep-widget deep-eicon-food-menu';
	...
	protected function render() {
		...
		if( !empty( $img ) ) {
			...
			$img = $image->m_image( $settings['img']['id'] , $img , '822' , '886' );
		}
		...
		if ( !empty( $settings['thumbnail']['width'] ) || !empty( $settings['thumbnail']['height'] ) ) {
			...
			$img = $image->m_image( $settings['img']['id'] , $settings['img']['url'] , $settings['thumbnail']['width'] , $settings['thumbnail']['height'] );
		}
		...
		switch ( $type ) {
			case '1': {
				?>
				<div class="wn-food-menu-sh food-menu-w<?php echo esc_attr( $type ); echo esc_attr( $shortcodeclass ); ?>"  <?php echo esc_attr( $shortcodeid ); ?>>
				<?php if( $img ): ?>
					<img src="<?php echo esc_url( $img ); ?>" alt="<?php echo esc_attr( $title ); ?>">
				<?php endif; ?>
					<div class="<?php echo esc_attr( $menu_class ); ?> space">
						<h3><?php echo esc_html( $title ); ?></h3>
						<h5><?php echo esc_html( $price ); ?></h5>
						<p>
						<?php
							foreach ( $ingredients as $line ) {
								$ingredients_data = isset( $line['ingredient'] ) ? $line['ingredient']: '';
								echo esc_html( $ingredients_data ) . '<br>' ;
							}
						?>
						</p>
					</div>
			...
			case '2': {
				?>
				<div class="wn-food-menu-sh food-menu-w2 <?php echo esc_attr( $shortcodeclass ); ?>"  <?php echo $shortcodeid; ?> >
				<?php
				...
				if ( $food_menu2 ) {
					foreach ( $food_menu2 as $line ) {
						...
						if( !empty( $image['url'] ) ) {
							...
							$food_image = new \Wn_Img_Maniuplate;
							$thumbnail = $food_image->m_image( $image['id'], $image['url'] , '20' , '20' );
							$datasrc = $food_image->m_image( $image['id'], $image['url'] , '650' , '650' );
						}
						...
						?>
						<div class="fm-w2-item<?php echo esc_attr( $food_style ); ?>">
							<div class="fm-w2-top space">
							<?php if ($image['url']): ?>
								<img src="<?php echo esc_url( $thumbnail ); ?>" <?php echo esc_attr( $data_src ); ?>>
							<?php endif;?>
								<span class="fm-w2-name"><?php echo esc_html( $title ); ?></span>
								<span class="fm-w2-price" ><?php echo esc_html( $price ); ?></span>
							</div>
							<div class="fm-w2-des"><?php echo esc_html( $description ); ?></div>
				...
				?>
					<div class="wn-food-menu-sh food-menu-w<?php echo esc_attr( $type ); echo esc_attr( $shortcodeclass ); ?>"  <?php echo esc_attr( $shortcodeid ); ?>>
						...
						<div class="<?php echo esc_attr( $menu_class ); ?> space">
							<h3><?php echo esc_html( $title ); ?></h3>
							<h5><?php echo esc_html( $price ); ?></h5>
							<?php echo esc_html( $ingredients_data ); ?>
						</div>
			...
			case '5': {
				$menu_class = 'menu-dts-'.$type ;
				?>
				<div class="wn-food-menu-sh food-menu-w<?php echo esc_attr( $type ); ?> colorb <?php echo esc_attr( $shortcodeclass ); ?>"  <?php echo esc_attr( $shortcodeid ); ?>>
					<div class="<?php echo esc_attr( $menu_class ); ?>">
						<h3><?php echo esc_html( $title ); ?></h3>
							<h5><?php echo esc_html( $price ); ?></h5>
					</div>
					<div class="border-<?php echo esc_attr( $type ); ?>">
						<?php if( $img ): ?>
							<img src="<?php echo esc_url( $img ); ?>" alt="<?php echo esc_attr( $title ); ?>" class="fmt-5">
						<?php endif; ?>
					</div>
					<div class="background-5">
						<p><strong><?php echo esc_html( $title ); ?> ingredients: </strong><?php echo wp_kses_post( $description ); ?></p>
					</div>
				</div>
				<?php
				break;
