...
class Webnus_Element_Widgets_Socials extends \Elementor\Widget_Base {
	...
	public function get_title() {
		...
		return esc_html__( 'Socials', 'deep' );
	...
	protected function register_controls() {
		...
		$this->add_control(
			'heading_type',
			[
				'label' => __( 'In order to add or remove social media items please navigate to Options -> Social Networks', 'deep' ),
	...
	protected function render() {
		...
		switch ($type) {
			case '0':
			echo '
			<div class="wn-socials-sh wn-social-network-type0 ' . $shortcodeclass . '"  ' . $shortcodeid . '>';

				if ( defined( 'DEEP_HANDLE' ) ) {
					include DEEP_SRC_DIR . 'templates/social.php';
				} else {
