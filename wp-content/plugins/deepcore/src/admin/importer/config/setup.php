...
use Deep\Deep_Core as Deep;
use Deep\Compatibility\Importer;
...
class Deep_Demo_Importer {
	...
	public function import_content() {
		...
		if ( ! class_exists( 'OCDI_Plugin' ) ) {
			require_once DEEP_SRC_DIR . 'admin/importer/one-click-demo-import/one-click-demo-import.php';
		}
		...
		foreach ( $im_content as $key => $item ) {
			switch ($item) {
				case 'pages':
					...
					$page_content_ele = file_get_contents( $this->path . '/pages-ele.xml' );
					...
					if ( ! Deep::is_deep_theme() ) {
						Importer::update_page_template( $page_content_ele );
					}
		...
		if ( class_exists( 'Woocommerce' ) ) {
			if ( file_exists( $this->path . '/products.xml' )  ) {
	...
	private function deep_api_demo_data() {
		...
		$url = 'http://webnus.site/demo-api/wp-json/dp/v1/deep-api/';
	...
	public static function deep_demo_listings_loop($pg = '', $atype = '', $acategory = '') {
		require_once DEEP_SRC_DIR . 'admin/importer/config/listings.php';
	...
	private static function deep_demo_checkbox($name, $slug, $imageURL, $type) {
		require_once DEEP_SRC_DIR . 'admin/importer/config/plugins.php';
		?>
			...
			<div class="wn-lightbox-contents">
				...
				<div class="wn-lightbox" slug="<?php echo $slug; ?>" type="<?php echo $type; ?>">
					...
					<div class="wnl-row import-btn-row">
						...
						<?php if ( ! Deep::is_deep_theme() ) : ?>
							<p class="w-importing">
								These demos are created for Deep theme.
								Since you're currently using another theme, depending on which theme you're using you may see some little issues that are related to the installed theme styles affecting the demo.
							</p>
						<?php endif; ?>
					...
					<div class="wn-lb-content wn-suc-imp-content-wrap">
						<div class="wn-suc-imp-content">
							...
							<div class="wn-suc-imp-links">
								...
								<a class="wn-suc-imp-links-t" href="<?php echo esc_url( self_admin_url( 'admin.php?page=wn-admin-options' ) ); ?>">Options</a>
